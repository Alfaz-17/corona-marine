      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredBlogs.map((blog, index) => (
            <motion.article
              key={blog.id}
              className="card bg-base-100 border border-cyan-100 shadow-md hover:shadow-xl hover:border-cyan-300 transition-all"
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
            ...
            >
              ...
              <div className="card-body">
                <h2 className="card-title text-navy-800 hover:text-cyan-600 transition-colors">
                  <Link to={`/blog/${blog.id}`}>{blog.title}</Link>
                </h2>

                <p className="text-cyan-700 mb-4">{blog.excerpt}</p>

                <div className="flex items-center gap-4 text-sm text-cyan-600 mb-4">
